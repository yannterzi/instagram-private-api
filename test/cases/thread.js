...
function shouldBeThreadItem(item) {
    ...
    item.params.type.should.be.oneOf(['text', 'media', 'mediaShare', 'actionLog', 'profile', 'location', 'hashtag', 'like', 'link', 'reel_share', 'placeholder']);
    ...
    if (item.params.type == "link") {
        item.link.should.be.instanceOf(Client.Link);
    }
}
